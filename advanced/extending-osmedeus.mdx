...
Utility functions are executed via the Otto JavaScript VM.
...
```go
func (r *OttoRuntime) myFunction(call otto.FunctionCall) otto.Value {
    // Get arguments
...
### 3. Register in Otto Runtime

In `internal/functions/otto_runtime.go`, add to `registerFunctionsOnVM()`:

```go
func (r *OttoRuntime) registerFunctionsOnVM(vm *otto.Otto) {

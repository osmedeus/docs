...
```
osmedeus-ng/
├── cmd/osmedeus/main.go              # Application entry point
├── internal/                         # Private packages
│   ├── config/config.go              # Config loading and defaults
│   ├── config/settings.go            # Settings struct definitions
│   ├── config/secrets.go             # Secret management
│   ├── core/workflow.go              # Workflow and RunnerConfig structs
│   ├── core/step.go                  # Step definition
│   ├── core/trigger.go               # Trigger types and events
│   ├── core/types.go                 # Status types, constants
│   ├── core/context.go               # Execution context
│   ├── core/constants.go             # Project metadata
│   ├── parser/parser.go              # YAML parser
│   ├── parser/loader.go              # Workflow loader with caching
│   ├── parser/validator.go           # Workflow validation
│   ├── executor/executor.go          # Main executor
│   ├── executor/dispatcher.go        # Step dispatcher
│   ├── executor/bash_executor.go     # Bash step handler
│   ├── executor/function_executor.go # Function step handler
│   ├── executor/foreach_executor.go  # Foreach step handler
│   ├── executor/parallel_executor.go # Parallel step handler
│   ├── executor/remote_bash_executor.go # Remote-bash step handler
│   ├── executor/http_executor.go     # HTTP request handler
│   ├── executor/llm_executor.go      # LLM step handler
│   ├── runner/runner.go              # Runner interface
│   ├── runner/host_runner.go         # Local execution
│   ├── runner/docker_runner.go       # Docker execution
│   ├── runner/ssh_runner.go          # SSH remote execution
│   ├── template/engine.go            # Template engine
│   ├── template/generators.go        # Value generators
│   ├── template/context.go           # Template context
│   ├── functions/registry.go         # Function registry
│   ├── functions/goja_runtime.go     # JavaScript runtime
│   ├── functions/file_functions.go   # File operations
│   ├── functions/string_functions.go # String operations
│   ├── functions/util_functions.go   # Utility functions
│   ├── functions/jq.go               # JSON query functions
│   ├── scheduler/scheduler.go        # Cron, event, watch triggers
│   ├── database/database.go          # Connection management
│   ├── database/models.go            # Data models
│   ├── database/jsonl.go             # JSONL import/export
│   ├── database/repository/          # Data access layer
│   ├── heuristics/heuristics.go      # Target type detection
│   ├── heuristics/url.go             # URL parsing
│   ├── heuristics/domain.go          # Domain analysis
│   ├── workspace/workspace.go        # Workspace creation
│   ├── snapshot/snapshot.go          # Export/import ZIP archives
│   ├── installer/installer.go        # Core installer logic
│   ├── installer/nix.go              # Nix package manager support
│   ├── installer/registry.go         # Binary registry management
│   ├── state/export.go               # State export functionality
│   ├── state/types.go                # State types
│   ├── updater/updater.go            # Update logic
│   ├── updater/github_source.go      # GitHub releases source
│   ├── terminal/printer.go           # Output formatting
│   ├── terminal/colors.go            # ANSI colors
│   ├── terminal/symbols.go           # Unicode symbols
│   ├── terminal/spinner.go           # Loading spinners
│   ├── terminal/table.go             # Table rendering
│   └── logger/logger.go              # Zap logger wrapper
├── pkg/                              # Public packages
│   ├── cli/root.go                   # Root command
│   ├── cli/scan.go                   # Scan command
│   ├── cli/workflow.go               # Workflow command
│   ├── cli/function.go               # Function command
│   ├── cli/server.go                 # Server command
│   ├── server/server.go              # Server setup
│   ├── server/handlers/              # Request handlers
│   └── server/middleware/            # Auth middleware (JWT, API Key)
├── test/                             # Test suites
│   ├── e2e/                          # E2E CLI tests
│   ├── integration/                  # Integration tests
│   ├── testdata/workflows/           # Test workflow fixtures
│   └── testdata/complex-workflows/   # Complex workflow examples
...
Functions are implemented in Go and exposed to a Goja JavaScript VM:
...
func (r *GojaRuntime) printf(call goja.FunctionCall) goja.Value

// catFile prints file content to stdout
func (r *GojaRuntime) catFile(call goja.FunctionCall) goja.Value

// exit exits the scan with given code (0=success, non-zero=error)
func (r *GojaRuntime) exit(call goja.FunctionCall) goja.Value
```

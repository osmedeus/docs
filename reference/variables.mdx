...
| `{{ExternalMarkdowns}}` | Alias for `{{MarkdownTemplates}}` |
...
| `{{run_id}}` | 8-character unique run identifier (lowercase) |
| `{{Output}}` | Output directory path for results |
| `{{HeuristicsCheck}}` | Heuristics analysis level (`basic`, `advanced`, or `none`) |

### Context Variables

Additional lowercase variables available in template context:

| Variable | Description |
|----------|-------------|
| `{{target}}` | Target being scanned (lowercase alias) |
| `{{workspace}}` | Workspace path |
| `{{base_folder}}` | Base installation folder (lowercase alias) |
...
| `{{StateConsoleLog}}` | Console log (`{{Output}}/run-console.log`) |
...
| Variable | Description | Example Value |
|----------|-------------|---------------|
| `{{TargetType}}` | Target type identifier | `"ip"` |
| `{{TargetRootDomain}}` | Original IP address | `192.168.1.100` |
